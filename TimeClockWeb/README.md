...
一般員工登入後的「我的紀錄」分頁**只會顯示自己的考勤卡**，看不到其他員工的紀錄；只有管理員登入後才能用下拉選單檢視全部員工並編輯。

管理員後台另有「**員工帳號密碼**」面板，可查看每位員工的姓名與登入密碼（手機號碼），供員工忘記時查詢；密碼預設遮蔽，點眼睛才顯示。
...
為避免任何人隨意用姓名＋手機就加入打卡名冊，第一次申請的帳號會進入「**待審核（pending）**」狀態：

- 新申請者登入後會看到「**等待管理員審核**」畫面，**還不能打卡、也不會出現在考勤名冊**。
- 管理員登入後，後台最上方會出現「**待審核申請**」區塊，列出每筆申請的姓名與手機，可按「**通過**」或「**拒絕**」。
- 按「通過」→ 帳號變成 active、加入名冊、即可打卡；申請者的畫面會自動切換成打卡畫面（靠既有的定時輪詢，不用重新登入）。
- 按「拒絕」→ 從名冊移除該申請；被拒絕者下次會回到登入畫面。
- 管理員自己在後台「新增員工」建立的帳號直接視為已審核（active），不需再經審核。
- 相容性：舊資料（沒有 `status` 欄位的員工）一律視為 active，既有帳號不受影響。

> 這個審核判斷發生在前端，屬於「流程上的防呆」，用途是避免陌生人自助加入名冊；它**不是**伺服器層級的權限控管（打卡 API 本身仍可被知道網址的人直接呼叫），完整的存取控管仍需另加有身份驗證的後端。
...
CSV 內容與畫面上的考勤卡採用同一套工時計算，並帶有 UTF-8 BOM，用 Excel 直接打開即可正確顯示中文。**工時欄位一律以「小時」為單位、半小時顯示為 `0.5`**（例如 8 小時半 → `8.5`、9 小時 → `9`），方便直接乘上時薪試算；上下班欄位仍是打卡時間（例如 `09:00`）。
